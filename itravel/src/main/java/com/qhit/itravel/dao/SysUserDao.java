...
public interface SysUserDao {
    ...
    SysUser findAllList(@Param("username") String username);

    @Select("select * from sys_user t where t.username = #{username}")
    SysUser getUser(String username);

    @Select("select * from sys_role r inner join sys_role_user s on r.id=s.roleId where userId = #{id}")
    ...
    void savaUserRoles(@Param("userId") Integer id,@Param("roleIds") List<Long> roleIds);
}